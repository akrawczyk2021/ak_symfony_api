...
class ShowCardParamConverter implements ParamConverterInterface
{
    ...
    {
        $card = $this->cardRepository->findOneByName($request->get('name'));
        if (!$card) {
            throw new NotFoundHttpException("No user found");
        }
        $showCard = new ShowCard($card->getName(), $card->getDescription(), $card->getAttack(), $card->getDefense(), $card->getHp());
